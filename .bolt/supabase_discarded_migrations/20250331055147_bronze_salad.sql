...
CREATE POLICY "Company admin read access"
  ...
  USING (
    CASE
      WHEN (auth_id = uid()) THEN true
      WHEN (EXISTS (
        SELECT 1
        FROM therapists admin
        WHERE ((admin.auth_id = uid()) AND (admin.role = 'company_admin') AND (admin.company_id = therapists.company_id))
      )) THEN true
      ELSE false
    END
  );
  ...
  USING (
    CASE
      ...
      WHEN (EXISTS (
        SELECT 1
        FROM therapists admin
        WHERE ((admin.auth_id = uid()) AND (admin.role = 'company_admin') AND (admin.company_id = therapists.company_id) AND (therapists.role <> 'company_admin'))
      )) THEN true
      ELSE false
    END
  )
